...
- The remediation sidebar includes a collapsed **Why this finding matters** principles panel below the approval action. It shows each applicable principle with its readable label, canonical doctrine name, and finding-specific evidence on demand; a compact **Principles** header disclosure gives the same at-a-glance context from the code pane.
