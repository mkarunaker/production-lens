...
- The remediation sidebar now includes an expanded **Why this finding matters** principles panel. It shows each applicable principle with its readable label, canonical doctrine name, and finding-specific evidence before approval; a compact **Principles** header disclosure gives the same at-a-glance context from the code pane.
